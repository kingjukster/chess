...
// These are the standard Polyglot zobrist keys
const uint64_t PolyglotBook::POLYGLOT_RANDOM[781] = {
...
PolyglotBook::PolyglotBook() 
    : num_entries(0), variety(50), min_weight(1), max_book_depth(20), book_ply(0) {
...
bool PolyglotBook::load(const std::string& fname) {
    close();
    ...
    num_entries = file_size / sizeof(PolyglotEntry);
    book_ply = 0;
...
uint64_t PolyglotBook::polyglot_zobrist_key(const Position& pos) const {
    uint64_t key = 0;
    uint64_t pieces_key = 0;
    uint64_t castling_key = 0;
    uint64_t ep_key = 0;
    uint64_t turn_key = 0;
    
    // Pieces
    // Polyglot piece encoding: black pawn=0, white pawn=1, black knight=2, ..., white king=11
    // Polyglot square: a1=0, h1=7, a8=56, h8=63 (same as engine: rank*8+file)
    for (Square sq = 0; sq < 64; sq++) {
        ...
        if (pc != NO_PIECE_PIECE) {
            ...
            // Polyglot: black=0, white=1 for each piece type (engine has WHITE=0, BLACK=1)
            int piece_idx = (pt - PAWN) * 2 + (1 - c);
            
            pieces_key ^= POLYGLOT_RANDOM[64 * piece_idx + sq];
        }
    }
    
    // Castling rights (indices 768-771)
    int cr = pos.castling_rights();
    if (cr & WHITE_OO) castling_key ^= POLYGLOT_RANDOM[768];
    if (cr & WHITE_OOO) castling_key ^= POLYGLOT_RANDOM[769];
    if (cr & BLACK_OO) castling_key ^= POLYGLOT_RANDOM[770];
    if (cr & BLACK_OOO) castling_key ^= POLYGLOT_RANDOM[771];
    
    // En passant file (indices 772-779)
    // Only hash if there's actually a pawn that can capture
    Square ep = pos.ep_square();
    if (ep != SQ_NONE) {
        // Check if there's a pawn that can actually capture
        // For now, always hash it if ep square is set
        ep_key ^= POLYGLOT_RANDOM[772 + file_of(ep)];
    }
    
    // Side to move (index 780)
    if (pos.side_to_move() == WHITE) {
        turn_key ^= POLYGLOT_RANDOM[780];
    }
    
    key = pieces_key ^ castling_key ^ ep_key ^ turn_key;
    
    return key;
}

Move PolyglotBook::polyglot_to_move(const Position& pos, uint16_t poly_move) const {
    // Polyglot move format (per spec: a1=0, same as engine):
    // bits 0-5: to square (to_file in 0-2, to_row in 3-5)
    ...
    if (promo == 1) promo_type = KNIGHT;
    else if (promo == 2) promo_type = BISHOP;
    else if (promo == 3) promo_type = ROOK;
    else if (promo == 4) promo_type = QUEEN;
    
    // Determine if capture
    bool is_capture = pos.piece_on(to) != NO_PIECE_PIECE;
    
    // Determine if special move (castling or en passant)
    bool is_special = false;
    Piece moving = pos.piece_on(from);
    
    if (type_of(moving) == KING && abs(file_of(from) - file_of(to)) > 1) {
        is_special = true;  // Castling
    } else if (type_of(moving) == PAWN && to == pos.ep_square()) {
        is_special = true;  // En passant
        is_capture = true;
    }
    
    return Move(from, to, promo_type, is_capture, is_special);
}

uint16_t PolyglotBook::move_to_polyglot(const Position& pos, Move move) const {
    int from = move.from();
    int to = move.to();
    ...
    if (move.is_promotion()) {
        ...
        if (pt == KNIGHT) promo = 1;
        else if (pt == BISHOP) promo = 2;
        else if (pt == ROOK) promo = 3;
        else if (pt == QUEEN) promo = 4;
    }
    
    return (promo << 12) | (from << 6) | to;
}

std::vector<PolyglotEntry> PolyglotBook::find_entries(uint64_t key) {
    std::vector<PolyglotEntry> entries;
    
    if (!is_loaded() || num_entries == 0) {
        return entries;
    }
    ...
    while (left < right) {
        ...
        file.seekg(mid * sizeof(PolyglotEntry));
        PolyglotEntry entry;
        file.read(reinterpret_cast<char*>(&entry), sizeof(PolyglotEntry));
        
        // Convert from big-endian
        uint64_t entry_key = __builtin_bswap64(entry.key);
    ...
    file.seekg(left * sizeof(PolyglotEntry));
...
std::vector<BookMove> PolyglotBook::probe(const Position& pos) {
    ...
    uint64_t key = polyglot_zobrist_key(pos);
    std::vector<PolyglotEntry> entries = find_entries(key);
    
    for (const auto& entry : entries) {
        Move move = polyglot_to_move(pos, entry.move);
        moves.push_back(BookMove(move, entry.weight));
    }
    
    return moves;
}

Move PolyglotBook::get_move(const Position& pos, bool best) {
    if (!is_loaded() || book_ply >= max_book_depth) {
        return MOVE_NONE;
    }
    
    std::vector<BookMove> moves = probe(pos);
    ...
    // If best move requested or variety is 0, return highest weighted move
    if (best || variety == 0) {
    ...
    // Higher variety = more uniform distribution
    uint64_t total_weight = 0;
    
    for (auto& bm : moves) {
        // Adjust weights based on variety
        // variety=0: use raw weights (best move heavily favored)
        // variety=100: uniform distribution
        if (variety == 100) {
            bm.weight = 1;
        } else {
            // Scale down weight differences based on variety
            double scale = 1.0 - (variety / 100.0);
            bm.weight = 1 + static_cast<uint16_t>(bm.weight * scale);
        }
